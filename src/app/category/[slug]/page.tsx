...
import { getCategory, getPostsByCategory, POST_TYPE_MAP, WordPressPost, extractPostMeta, getDeadlineStatus } from '@/lib/wordpress';
...
export async function generateMetadata({ params }: CategoryArchiveProps): Promise<Metadata> {
  ...
  return {
    title: `Latest ${categoryName} Updates 2026 – Jobs & Notifications | Get Job Update`,
    description: `Latest notifications, active vacancies, and updates for ${categoryName}. Apply online, check eligibility, and get direct links.`,
    alternates: {
...
export default async function CategoryArchivePage({ params }: CategoryArchiveProps) {
  ...
  try {
    posts = await getPostsByCategory(slug, 30);
  } catch (err) {
    console.error(`Failed to fetch category posts for: ${slug}`, err);
  }
  ...
  const schemas = [
    {
      ...
      "@type": "CollectionPage",
      "name": `Latest ${categoryName} Updates 2026`,
      "description": `Active government jobs, admit cards, and results for ${categoryName} category.`,
      "url": `https://getjobupdate.co.in/category/${slug}`,
      "mainEntity": {
        "@type": "ItemList",
    ...
    },
    {
      ...
      "itemListElement": [
        ...
        { "@type": "ListItem", "position": 2, "name": "Categories", "item": "https://getjobupdate.co.in/categories" },
        { "@type": "ListItem", "position": 3, "name": categoryName, "item": `https://getjobupdate.co.in/category/${slug}` }
      ]
    }
  ];
    ...
    <>
      ...
      <div className="w-full bg-slate-50 min-h-screen flex flex-col font-sans">
        ...
        <div className="bg-slate-900 w-full pt-16 pb-28 md:pb-36 relative overflow-hidden">
          ...
          <div className="absolute top-0 right-0 w-80 h-80 bg-orange-600/8 rounded-full blur-3xl pointer-events-none" />
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-10 text-center">
            ...
            <h1 className="text-4xl md:text-5xl lg:text-[3.5rem] font-black text-white leading-[1.1] tracking-tight mb-5 flex items-center justify-center gap-3">
              ...
              <span className="text-indigo-400">{categoryName} Updates</span>
            </h1>
            <p className="text-slate-400 text-sm md:text-base font-medium mb-4 max-w-2xl mx-auto leading-relaxed">
              Explore all active vacancies, government exams, result notifications, and summaries announced under the {categoryName} category.
            </p>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 w-full -mt-12 md:-mt-20 relative z-20 pb-12">
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                ...
                <div className="flex items-center gap-2">
                  <select className="bg-slate-50 border border-slate-200 text-slate-700 text-sm rounded-lg px-3 py-2 outline-none focus:border-indigo-500 cursor-pointer transition-colors">
              ...
              {posts.length > 0 ? (
                  ...
                  {posts.map((post) => {
                    ...
                    return (
                      <Link 
                        ...
                        className="group flex flex-col bg-white rounded-2xl border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.05)] hover:shadow-[0_16px_40px_rgba(0,0,0,0.08)] hover:-translate-y-1 hover:border-indigo-200 transition-all duration-300 overflow-hidden h-full relative"
                      >
                        {/* Hover Gradient Accent Line */}
                        <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-indigo-400 to-cyan-400 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                        
                        <div className="p-5 flex flex-col flex-1">
                          
                          <div className="flex items-start justify-between gap-2 mb-3">
                            <span className="inline-flex items-center gap-1.5 rounded-md bg-indigo-50 px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-indigo-700 border border-indigo-100">
                              {categoryName}
                            </span>
                            {isNew && (
                          ...
                          <h3 
                            className="text-[15px] font-bold text-slate-900 group-hover:text-indigo-600 transition-colors leading-snug line-clamp-3 mb-4"
                ...
                <div className="p-16 flex flex-col items-center justify-center text-center bg-white border border-dashed border-slate-300 rounded-2xl">
                  ...
                  <p className="text-slate-500 text-sm max-w-sm">
                    We currently don't have any active postings under this category. Please check back later or subscribe to notifications.
                  </p>
                </div>
              )}
          ...
          <div className="mt-16 bg-white rounded-2xl border border-slate-100 shadow-[0_4px_20px_rgba(0,0,0,0.05)] p-6 md:p-10 mb-12 relative overflow-hidden">
            ...
            <h2 className="text-2xl font-black text-slate-900 mb-4 tracking-tight">Updates in {categoryName}</h2>
            <div className="prose prose-sm md:prose-base max-w-none text-slate-600 space-y-4">
              <p>Welcome to the dedicated portal for <strong>{categoryName} Updates</strong> in India. At GetJobUpdate, we understand that finding roles tailored to your exact career path is crucial. Our system actively categorizes vacancies specifically for candidates seeking opportunities in {categoryName}.</p>
              <p>Ensure you never miss a deadline by bookmarking this page. Every vacancy listed above is cross-verified directly with official recruitment boards. Click on any job to view detailed age limits, exact eligibility criteria, syllabus, and official apply online links.</p>
            </div>
