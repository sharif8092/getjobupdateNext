...
// ─────────────────────────────────────────────
// STATIC DATA — no API needed, renders instantly
// ─────────────────────────────────────────────
...
// These stream in progressively without blocking the LCP element
// ─────────────────────────────────────────────
...
async function FeedGrid() {
  ...
  let privateJobs: WordPressPost[] = [];
  let wfhJobs: WordPressPost[] = [];
  ...
  try {
    ...
    privateJobs = await getPostsByCategory('private-job', 4);
    wfhJobs = await getPostsByCategory('work-from-home', 4);
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
      ...
      />
      <FeedCard title="Private Jobs" typeSlug="category/private-job" posts={privateJobs} accentColor="text-indigo-400"
      ...
      />
      <FeedCard title="Work From Home" typeSlug="category/work-from-home" posts={wfhJobs} accentColor="text-teal-400"
        iconBg="bg-teal-500" hoverText="group-hover:text-teal-600"
...
// ─────────────────────────────────────────────
// STATIC SECTIONS (no data needed — render instantly with LCP)
// ─────────────────────────────────────────────
...
const categories = [
  ...
  { href: '/category/private-job', label: 'CORPORATE ROLES', name: 'Private Job', iconBg: 'bg-blue-100 text-blue-600', icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 21h16.5M4.5 3h15M5.25 3v18m13.5-18v18M9 6.75h1.5m-1.5 3h1.5m-1.5 3h1.5m3-6h1.5m-1.5 3h1.5m-1.5 3h1.5M9 21v-2.25A2.25 2.25 0 0111.25 16.5h1.5a2.25 2.25 0 012.25 2.25V21" /></svg> },
  { href: '/category/work-from-home', label: 'REMOTE WORK', name: 'Work from Home', iconBg: 'bg-cyan-100 text-cyan-600', icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 12l8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25" /></svg> },
...
// ─────────────────────────────────────────────
// MAIN PAGE — Hero + Category links render IMMEDIATELY (no data wait)
    ...
    <>
      ...
      <div className="bg-slate-900 w-full pt-16 pb-36 relative overflow-hidden z-0">
        ...
        <div className="max-w-4xl mx-auto text-center px-4 relative z-10">
          ...
          {/* LCP ELEMENT — this is what Google measures. Renders in first byte of HTML. */}
      ...
      {/* ════════════════════════════════
          2. CATEGORY QUICK LINKS — static, renders instantly
      ════════════════════════════════ */}
      ...
      {/* ════════════════════════════════
          3. MAIN FEEDS — Suspense streaming
          Skeleton shown instantly; real data streams in when API responds
      ════════════════════════════════ */}
      ...
      {/* ════════════════════════════════
          4. CAREER PATHWAYS — below fold, cv-auto
      ════════════════════════════════ */}
